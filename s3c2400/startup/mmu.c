...
typedef unsigned32 mmu_lvl1_t;

extern unsigned32 _ttbl_base;

static inline unsigned32 mmu_get_id(void);
static inline unsigned32 mmu_get_ctrl(void);
static inline void mmu_set_ctrl(unsigned32 val);
static inline unsigned32 mmu_get_trans_tbl(void);
static inline void mmu_set_trans_tbl(unsigned32 val);
static inline unsigned32 mmu_get_domain_ctrl(void);
static inline void mmu_set_domain_ctrl(unsigned32 val);
static inline unsigned32 mmu_get_fault_stat(void);
static inline void mmu_set_fault_stat(unsigned32 val);
static inline unsigned32 mmu_get_fault_addr(void);
static inline void mmu_set_fault_addr(unsigned32 val);
static inline void mmu_set_cache_inval(void);
static inline void mmu_set_tlb_inval(void);
static inline unsigned32 mmu_get_proc_id(void);
static inline void mmu_set_proc_id(unsigned32 val);
...
void mmu_init(mmu_sect_map_t *map, int num) 
{
    ...
    mmu_set_trans_tbl((unsigned32) lvl1_base);
    ...
    for (i = 0; i < num; i++) {
        ...
        while (sects > 0) {
            lvl1_base[pbase] = MMU_SET_LVL1_SECT(vbase << 20,
                                                 MMU_SECT_AP_ALL, 
                                                 0, 
                                                 c, 
                                                 b);
